    ...
    public void execute() {
        ...
        */
        case 0: // Init Trajectory
            ...
            setExt(pidHdg, hdgSP, 5.0, 0.5, 1.0, 2.0, true);
            ...
            //Set extended values pidCtlr, SP, DB, Mn, Mx, Exp, Cmp
            setExt(pidDist, radiusSP, 0.0, 0.0, 0.5, 2.0, true);
        ...
        case 1: // Turn to heading.  Mx spd on out whl.  Hold radius on inside whl.
            strCmd[0] = pidHdg.calculateX(hdgFB());              //cmd[0]=fwd(Outside)
            strCmd[1] = pidDist.calculateX(radiusFB(ctrX, ctrY));//cmd[1]=radius(Inside)
            strCmd[0] += strCmd[1];
            strCmd[1] = 0.0;
        ...
        case 2: // Done
            ...
            System.out.println("CHT - 2: Final Inside Cmd: " + strCmd[turnRight ? 1 : 0]);
            break;
        }
        updSDB();
    }
