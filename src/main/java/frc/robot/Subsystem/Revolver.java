...
public class Revolver {
    ...
    public static void update() { // cases for state of revolver
        ...
        case 1: // waiting for intake, snorfling
            cmdUpdate(0.0);
            if(delayTimer.hasExpired(0.5, state)) state++;  //Settling time
            break;
        ...
        case 91: // Reverse momentarily before trying again.
            ...
            cmdUpdate(-loadPct);
            if(delayTimer.hasExpired(0.5, state) ) state++;
            break;
    ...
    private static void stfUpdate() {
        if(IO.revolver_HAA) jammedBall = true;  // Try to clear by reversing
