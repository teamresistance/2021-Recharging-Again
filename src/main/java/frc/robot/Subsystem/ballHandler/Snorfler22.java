...
public class Snorfler22 {
    ...
    // //Defines RevRobotics 3 color sensor and RGB colors to sensr ball colors, Red
    // & Blue.
    ...
    // private static final Color kBlueTarget = new Color(0.17, 0.41, 0.41); //
    // blue: R-17 G-41 B-41
    // private static final Color kGreenTarget = new Color(0.197, 0.561, 0.240);
    // private static final Color kRedTarget = new Color(0.49, 0.35, 0.15); // red
    // R-49 G-35 B-15
    ...
    public static void update() {
        ...
        if ((btnSnorfle.isDown() || reqsnorfDrvAuto) && state == 0)
            state = 1; // Starts the state machine
        if ((btnSnorfle.isUp() && !reqsnorfDrvAuto) && state != 0)
            state = 0;
        if (btnRejectSnorfle.isDown() && state == 2)
            state = 3;
        if (btnRejectSnorfle.isUp() && state == 4)
            state = 0; // Goes back to off
    ...
    private static void smUpdate() { // State Machine Update
        ...
        switch (state) {
            ...
            case 4: // Go back and reject
                ...
                // if (btnRejectSnorfle.isUp() || btnSnorfle.isUp()) state = 0; // Goes back to
                // off
                break;
            default: // all off
                cmdUpdate(false, dirSnorfler.OFF);
